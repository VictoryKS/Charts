defmodule Statistic.GanttChart do
  @moduledoc """
  ...
  alias __MODULE__
  alias Statistic.{Scale, OrdinalScale, TimeScale, CategoryColourScale}
  ...
  Colours can either be a named palette defined in `Statistic.CategoryColourScale` or a list of strings representing hex code
  ...
  `mapping: %{category_col: :category, task_col: :task_name, start_col: :start_time, finish_col: :end_time, id_col: :task_id}`
  """
  @spec new(Statistic.Dataset.t(), keyword()) :: Statistic.GanttChart.t()
  ...
  @spec set_default_scales(Statistic.GanttChart.t()) :: Statistic.GanttChart.t()
  ...
  @spec show_task_labels(Statistic.GanttChart.t(), boolean()) :: Statistic.GanttChart.t()
  ...
  @spec set_category_task_cols(
          Statistic.GanttChart.t(),
          Statistic.Dataset.column_name(),
          Statistic.Dataset.column_name()
        ) ::
          Statistic.GanttChart.t()
  ...
  @spec set_task_interval_cols(
          Statistic.GanttChart.t(),
          {Statistic.Dataset.column_name(), Statistic.Dataset.column_name()}
        ) ::
          Statistic.GanttChart.t()
  ...
  """
  @deprecated "Use `:mapping` option in `new/2`"
  @spec set_id_col(Statistic.GanttChart.t(), Statistic.Dataset.column_name()) :: Statistic.GanttChart.t()
  ...
  def to_svg(%GanttChart{} = plot, _options) do
    ...
    height = get_option(plot, :height)
    time_axis = Axis.new(time_scale, :bottom) |> Axis.set_offset(height)
    toptime_axis = Axis.new(time_scale, :top) |> Axis.set_offset(height)
  ...
  defp get_category_rects_svg(
         %GanttChart{mapping: mapping, dataset: dataset, category_scale: _cat_scale} = plot
       ) do
  ...
  defp get_category_tick_svg(text, {_min_y, max_y}), do:
    [
      ~s|<g class="exc-tick" font-size="10" text-anchor="start" transform="translate(0, #{max_y})">|,
