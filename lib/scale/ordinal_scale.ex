defmodule Statistic.OrdinalScale do
  alias __MODULE__
  ...
  def update_transform_funcs(%OrdinalScale{domain: domain, range: {start_r, end_r}, padding: padding} = scale)
    ...
    domain_to_range_fn = fn domain_val ->
      case Enum.find_index(domain, fn x -> x == domain_val end) do
        nil -> start_r
        index -> start_r + item_width / 2.0 + index * item_width
      end
    end
        ...
        _ ->
          fn range_val ->
            case domain_count do
              0 -> ""
              _ ->
                bucket_index = trunc((range_val - start_r) / item_width)
  ...
  defimpl Statistic.Scale do
    ...
    def domain_to_range_fn(%OrdinalScale{domain_to_range_fn: domain_to_range_fn}), do:
      domain_to_range_fn

    def domain_to_range(%OrdinalScale{domain_to_range_fn: transform_func}, range_val)
      when is_function(transform_func), do: transform_func.(range_val)
